...
const InsuranceBox = ({ data }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const query = await axios.get(`${apiURL}/api/update/insurancecomment/${data.uniqueStr}/${comment}`)
            alert("Comment added");
        } catch (error) {
            console.log(error)
        }
    ...
    const handleHandledBySubmit = async () => {
        try {
            const query = await axios.get(`${apiURL}/api/update/insurancehandledBy/${data.uniqueStr}/${handle}`)
