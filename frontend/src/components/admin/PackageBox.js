...
const PackageBox = ({ data }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const query = await axios.get(`${apiURL}/api/update/packagecomment/${data.uniqueStr}/${comment}`)
            alert("Comment added");
        } catch (error) {
            console.log(error)
        }
    ...
    const handleHandledBySubmit = async () => {
        try {
            const query = await axios.get(`${apiURL}/api/update/packagehandledBy/${data.uniqueStr}/${handle}`)
